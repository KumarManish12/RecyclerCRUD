...
class MainActivity : AppCompatActivity(), ClickList {
    ...
    var shownUserModel = ArrayList<UserModel>()
...
//            }
        ...
        adapter = UserAdapter(shownUserModel, this)
        binding.recycleview.adapter = adapter
        binding.recycleview.layoutManager = LinearLayoutManager(this)
        binding.etsearch.doOnTextChanged { text, start, before, count ->
            if ((text ?: "").isEmpty()) {
                shownUserModel.addAll(userList)
            }else if ((text?:"").length >2){
                shownUserModel.clear()
                var list= java.util.ArrayList<UserModel>()
                list.addAll(userList.filter { element -> element.name?.contains((text?:""),ignoreCase = true)==true })
                shownUserModel.addAll(list)
           }
            }
            binding.btnok.setOnClickListener {
                ...
                dialogBinding.btnAdd.setOnClickListener {
                    if (dialogBinding.etName.text.toString().isEmpty()) {
                        ...
                        val userModel = UserModel(dialogBinding.etName.text.toString(),
                        ...
                        db.collection("User")
                            ...
                            .addOnFailureListener {
                                System.out.println("in error ${it.message.toString()}")
                                Toast.makeText(this,"Data does not added ${it.message.toString()}" ,Toast.LENGTH_LONG).show()
                            }
